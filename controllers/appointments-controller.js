...
router.get("/getAppointmentsByBuilding/:buildingId", (req, res) => {
  ...
  if (found) {
    var index = appointments
      .map((appointment) => appointment.buildingId)
      .indexOf(parseInt(req.params.buildingId));
    if (index !== -1) {
      appointments.splice(index, 1);
    }
  } else {
    res
      .status(400)
      .json({ msg: `No member with the id of ${req.params.buildingId}` });
  }
});

router.get("/getAppointmentaByBoiler/:boilerId", (req, res) => {
  const found = appointments.some(
    (appointments) => appointments.boilerId === parseInt(req.params.boilerId)
  );
  ...
  if (found) {
    ...
    res.status(400).json({
      msg: `No boiler type with the boilerId of ${req.params.boilerId}`,
    });
  }
});
...
router.get("/getAppointmentsStart/:start_timestamp", (req, res) => {
  ...
  if (found) {
    ...
    res.status(400).json({
      msg: `No boiler type with the start_timestamp of ${req.params.start_timestamp}`,
    });
  }
});
...
router.get("/getAppointmentsEnd/:end_timestamp", (req, res) => {
  const found = appointments.some(
    (appointments) => appointments.end_timestamp === req.params.end_timestamp
  );
  ...
  if (found) {
    ...
    res.status(400).json({
      msg: `No boiler type with the stock of ${req.params.end_timestamp}`,
    });
  }
});
