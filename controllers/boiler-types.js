...
exports.update = (req, res) => {
  ...
  BoilerTypes.findOneAndUpdate({ id }, req.body, { useFindAndModify: false })
    .then(data => {
      if (!data) {
        res.status(404).send({
          message: `Cannot update Boiler Type ID "${id}". Maybe Boiler Type was not found!`
        });
      } else {
        res.send({
          message: `Boiler Type ID "${id}" was updated successfully`
        });
      };
    });
};
...
exports.delete = (req, res) => {
  ...
  BoilerTypes.findOneAndRemove({ id }, { useFindAndModify: false })
    .then(data =>
      res.send({ message: 'Boiler Types was removed successfully' })
    )
    .catch(err => {
      res.status(500).send({
        message:
